...
// Controls whether we are looking at the List or the visual Calendar grid
enum ViewMode { case list, calendar }
// Controls whether the Calendar is showing 1 day or 5 days
...
struct ContentView: View {
        ...
        NavigationView {
            ZStack {
                ...
                if !networkManager.isAuthenticated {
                    ...
                    mainBodyView
                        // This applies the floating tab bar to the bottom of the screen
            ...
            // Clickable Date Range Menu (Hides Week ID)
            .toolbar {
    ...
    private var mainBodyView: some View {
        VStack(spacing: 0) {
            // If in Calendar mode, show the Day vs Week toggle
            ...
            // If looking at a specific day, show the Day selector
    ...
    private var liquidGlassTabBar: some View {
        ...
        .clipShape(Capsule())
        // Subtle drop shadow to make it float:
    ...
    private func tabButton(title: String, icon: String, mode: ViewMode) -> some View {
        Button(action: {
            ...
            HStack(spacing: 6) {
                ...
                // Hide text of the unselected tab for a dynamic expanding effect
                if viewMode == mode {
                    Text(title)
    ...
    private func calendarGridView(for schedule: WeekSchedule) -> some View {
        ...
        return ScrollView([.vertical, .horizontal]) {
                            ...
                            if let classes = schedule.days[day] {
                                ForEach(classes, id: \.self) { session in
                                    ...
                                    calendarBlock(for: session)
                                        ...
                                        .offset(y: offset)
                                        .onTapGesture {
                                            CalendarManager.shared.addClassToCalendar(session: session)
                                        }
                                }
                            }
                        }
            ...
            .padding(.horizontal)
            .padding(.bottom, 40) // Extra padding so content isn't hidden under tab bar
        }
    }
    ...
    private func classCard(for session: ClassSession) -> some View {
        HStack {
            ...
            Button(action: { CalendarManager.shared.addClassToCalendar(session: session) }) {
                Image(systemName: "calendar.badge.plus").foregroundColor(.blue).font(.title3)
            }
